...
int main(int argc, char* argv[])
{
	...
	std::copy_if(words.begin(), words.end(), std::back_inserter(palindromes), is_palindrome);

	//std::vector<std::string> tester = { "dad", "bob", "not", "aaa" };
	//tester.push_back("tacocat");
	//std::copy_if(tester.begin(), tester.end(), std::back_inserter(palindromes), is_palindrome);
	...
	//std::cout << "Palindrome size: " << palindromes.size() << "\n";

   //iterate over your sorted palindromes and print each one, one per line. Run my example for
   //expected output.

	for (std::vector<std::string>::iterator it = palindromes.begin(); it != palindromes.end(); ++it)
	{
	   std::cout << *it << "\n";
	}
