...
using RadarCallback_onNewBlueMissile = std::function<void (unsigned int)>;
...
public:
   ...
   */
   ...
   void setIlluminationFunc(const RadarCallback_onObjectIlluminatedByRadar& onRadarIlluminationOccurredCallback);
   void subscribeToUpdateBlueMissiles(const RadarCallback_onNewBlueMissile& addBlueMissileCallback);
   ...
   float getScanRangeMeters();
  // std::vector<std::pair<float, float>> makeObservedPoints();
   std::optional<std::vector<std::pair<float, float>>> makePredictedPoints(unsigned int id);

   void resetMissilePoints();

   void addAssignedMissile(unsigned int id);
   void removeAssignedMissile(unsigned int id);
   bool isNewMissile() const; //will be used to alert glview whenever it needs to check if there is a new missile sensed
   void addTargetMissile(unsigned int id);//this is a function that will be called within the radar sensor to update the red missiles
   void addMissilePoints(std::pair<unsigned int, std::tuple<Vector, double> >);
   std::optional<std::pair<std::tuple<double, double>, std::tuple<Vector, Vector>>> getTimesAndPoints(unsigned int id);
   bool checkForCollision(unsigned int, unsigned int);
   void addRedMissileAssigned(unsigned int id);
   void setRotationSpeed(float rotationsPerSec);
   float getRotationsPSec();
   //std::pair<std::vector<Vector>, std::vector<double>> getMissilePoints(unsigned int id);
   int getKilledMissiles();
   void increaseKilledMissiles();
   ...
   std::multimap<unsigned int, std::tuple<Vector, double> > missilePoints;
...
private:
   ...
   //these are two parallel vectors for the blue and red missiles
   std::vector<unsigned int> targetedMissiles = {};
   std::vector<unsigned int> assignedMissiles;
   std::vector<std::tuple<unsigned int, bool>> areMissilesAssigned;
   bool newMissile = true;
   const int blastRaduis = 10;
   float rotPerSecond = 0;
   int missileKills = 0;

   std::vector<unsigned int> getUnassignedMissileId();
   ...
   RadarCallback_onNewBlueMissile callback_onNewBlueMissile;
};
