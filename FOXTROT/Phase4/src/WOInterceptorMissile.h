...
/** 
   ...
   \{
*/
using  MissileCallback_onCollisionDetected = std::function<bool( unsigned int, WO* )>;
...
public:
   ...
   */
   ...
   void subscribeToCollisionDetection(const MissileCallback_onCollisionDetected detectedMissileCollision);
   void onCollision();

   std::vector<double> getBlueTrajectory(std::pair<std::tuple<double, double>, std::tuple<Vector, Vector>> timepoints, double protectionRad);
   double blast_radius = 2.5;
...
protected:
   ...
   WOTrackingRibbonBase* missileRibbon = nullptr;
   bool detectedRadarPulse = false;
   MissileCallback_onCollisionDetected callback_onCollisionDetected = nullptr;
